...
namespace Lsc {
    ...
    class IOSTR_LIB String {
        ...
        struct Format {
            ...
            template<typename T>
            bool Input(const T &aArg) {
                if (!LocateArg()) return false;
                string Str;
...
//            }
        ...
        */
        std::string::iterator begin() { return _mStr.begin(); }

        std::string::iterator end() { return _mStr.end(); }
        // --------------------------
    private:
